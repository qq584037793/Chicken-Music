  ...
  <teleport to="body">
    <transition name="slide">
      <div class="add-song" v-show="visible">
        <div class="header">
          <h1 class="title">曲を追加する</h1>
          <div class="close" @click="hide">
            <i class="icon-close"></i>
          </div>
        </div>
        <div class="search-input-wrapper">
          <search-input v-model="query" placeholder="曲を検索"></search-input>
        </div>
        <div v-show="!query">
          <switches :items="['最近再生', '検索履歴']" v-model="currentIndex"></switches>
        ...
        <message ref="messageRef">
          <div class="message-title">
            <i class="icon-ok"></i>
            <span class="text">プレイリストに1曲追加しました</span>
          </div>
        </message>
      </div>
    </transition>
